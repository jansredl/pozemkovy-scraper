...
def scrape():
    ...
    for a in links:
        ...
        # pokus o klíčová slova z textu
        site_cov = "čov" in text.lower() or "čistírna" in text.lower()
        site_kanalizace = "kanalizace" in text.lower()

        parcela_match = re.search(r"parcela\s*č(?:\.|íslo)?\s*([\w\-/]+)", text.lower())
        uzemni_plan_match = re.search(r"(https?://[\w./-]*uzemni[\w./-]*)", text.lower())
        ...
        results.append({
            ...
            "sit_cov": site_cov,
            "sit_kanalizace": site_kanalizace,
            "cislo_parcely": parcela_match.group(1) if parcela_match else None,
